...
const pipeline = new GenerationPipeline(new LocalLlmProvider(), new LocalTtsProvider(), new LocalAssetProvider(), new LocalRenderProvider());
export async function queueGeneration(generationId, projectId, userId) {
    setTimeout(async () => {
        try {
            ...
            markGenerationFailed(generationId, error instanceof Error ? error.message : "Generation failed");
        }
    }, 50);
}
