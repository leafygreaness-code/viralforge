...
import { enrichProjectInput } from "../services/source-service.js";
...
projectsRouter.post("/", async (req, res) => {
  const userId = String(req.headers["x-user-id"] ?? "demo-user");
  const parsedInput = projectSchema.parse(req.body);
  const input = await enrichProjectInput(parsedInput);
